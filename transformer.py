...
class LayerNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.a1 * (x - mean) / (std + self.eps) + self.b2
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, mask):
        x = self.sublayer[0](x, lambda k: self.self_attn(k, k, k, mask))
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, x, memory, source_mask, target_mask):
        x = self.sublayer[0](x, lambda k: self.self_attn(k, k, k, target_mask))
        x = self.sublayer[1](x, lambda k: self.src_attn(k, memory, memory, source_mask))
...
class EncoderDecoder(nn.Module):
    ...
    def forward(self, source, target):
        source_mask = mask_pad(source)
        target_mask = mask_tril(target)
